...
public final class RichInputConnection {
        ...
        if (clipboard != null && clipboard.hasPrimaryClip()) {
            ...
            if (clipData != null && clipData.getItemCount() >= 1) {
                ...
                if (pasteData != null && pasteData.length() > 0) {
                    // Use SmartPasteEngine for lossless large-text insertion
                    pasteFromHistory(pasteData.toString());
                    return;
                }
            }
        }
    ...
    /**
     * Insert full text using the smart paste engine (handles large payloads safely).
     */
    public void pasteFromHistory(final String fullText) {
        if (fullText == null || fullText.isEmpty() || mIC == null) return;
        rkr.simplekeyboard.inputmethod.paste.SmartPasteEngine.paste(mIC, fullText, null);
    }
